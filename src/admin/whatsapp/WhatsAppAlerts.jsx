...
const WhatsAppAlerts = ({ t }) => {
  ...
  const toggleAlert = (alertType) => {
    setAlerts((prev) => prev.map((a) =>
      a.alert_type === alertType ? { ...a, enabled: !a.enabled } : a
    ));
  };

  const updateTemplate = (alertType, value) => {
    setAlerts((prev) => prev.map((a) =>
      a.alert_type === alertType ? { ...a, template_message: value } : a
    ));
  };
  ...
  if (loading) {
    return <div style={cardStyle}><p style={{ color: '#64748b' }}>Carregando alertas...</p></div>;
  }
    ...
    <div style={cardStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <button onClick={handleSave} disabled={saving} style={{
          ...
          fontWeight: 700, fontSize: 13, fontFamily: 'inherit', opacity: saving ? 0.6 : 1,
        }}>
      ...
      <p style={{ fontSize: 12, color: '#64748b', marginBottom: 16 }}>
        {t('whatsappAlertsDesc') || 'Selecione quais alertas do Traccar serão enviados via WhatsApp. Use {device}, {event}, {time}, {data} nos templates.'}
      </p>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <label style={{ position: 'relative', width: 44, height: 24, cursor: 'pointer' }}>
                  ...
                  <span style={{
                    position: 'absolute', top: 2, left: alert.enabled ? 22 : 2,
                    width: 20, height: 20, borderRadius: '50%',
                    background: '#fff', transition: 'left 0.2s',
                    boxShadow: '0 1px 3px rgba(0,0,0,0.3)',
                  }} />
                </label>
                <span style={{ fontSize: 14, color: alert.enabled ? '#e2e8f0' : '#64748b', fontWeight: 500 }}>
                  {alert.label}
                </span>
              </div>
              <button
                onClick={() => setEditingType(editingType === alert.alert_type ? null : alert.alert_type)}
                style={{
            ...
            {editingType === alert.alert_type && (
              <div style={{ padding: '12px 14px', marginTop: 4 }}>
                <textarea
                  ...
                  rows={3}
                  style={{
                    width: '100%', padding: 10, borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)',
